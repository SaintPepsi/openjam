 ...
 *   .showError(msg) / .showPii(msg) / .clearNotices()
...
(function () {
  ...
  TPL.innerHTML = [
    ...
    "    <div class='notice pii' hidden></div>",
  ...
  class OpenJamPopup extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._elPii = r.querySelector(".pii");
    ...
    attributeChangedCallback() { if (this._elToggle) this._syncLabels(); }
    ...
    showError(msg) { this._elErr.innerHTML = msg || ""; this._elErr.hidden = !msg; }
    showPii(msg) { this._elPii.textContent = msg || ""; this._elPii.hidden = !msg; }
    clearNotices() { this._elErr.hidden = true; this._elPii.hidden = true; }
    ...
    _startDemo() {
      ...
      this._demoTimer = setInterval(() => {
        if (this.recording) { this._elapsed += 1000; this._syncLabels(); }
      }, 1000);
    }
    ...
    _demoMic(on) {
      ...
      // a gentle audible demo when the user turns the mic ON (a real gesture)
      if (on && !this._reduce) this._playDemoAudio();
      this._syncLabels();
    }
    ...
    _playDemoAudio() {
      try {
        ...
        var t0 = ctx.currentTime + 0.02, dur = 3.4;
        var master = ctx.createGain(); master.gain.value = 0.0001; master.connect(ctx.destination);
        var osc = ctx.createOscillator(); osc.type = "sawtooth"; osc.frequency.value = 116;
        var lfo = ctx.createOscillator(); lfo.frequency.value = 5.2;
        var lfoG = ctx.createGain(); lfoG.gain.value = 6; lfo.connect(lfoG); lfoG.connect(osc.frequency);
        var lp = ctx.createBiquadFilter(); lp.type = "lowpass"; lp.frequency.value = 900;
        var vg = ctx.createGain(); vg.gain.value = 0;
        osc.connect(lp); lp.connect(vg); vg.connect(master);
        var g = vg.gain; g.setValueAtTime(0.0001, t0);
        for (var i = 0; i < 24; i++) {
          var tt = t0 + (i / 24) * dur;
          var e = Math.max(0.05, Math.pow(Math.abs(Math.sin(i * 0.9)), 1.4));
          g.linearRampToValueAtTime(0.0001 + e * 0.9, tt);
        }
        g.linearRampToValueAtTime(0.0001, t0 + dur);
        master.gain.setValueAtTime(0.0001, t0);
        master.gain.linearRampToValueAtTime(0.08, t0 + 0.1);
        master.gain.setValueAtTime(0.08, t0 + dur - 0.15);
