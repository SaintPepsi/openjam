...
5. **Design** — choose one approach and record the design. Name any new method, field, class, or CSS hook for the concept it represents, not for today's single use: simple, descriptive, intuitive, no abbreviations, so the name still fits when a related feature lands later (a generic warning slot named `pii` forces a rename the moment a real PII feature arrives). **Gate:** a note reading `design: <chosen> — beats <runner-up> because <reason>`; flagged for Ian's ratification in the report, per the fork rule.
